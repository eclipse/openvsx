...
public abstract class TrustedPublishingProviderSupport {
    ...
    protected TrustedPublishingProviderSupport(TrustedPublishingConfig config,
                                               String providerId,
                                               String providerName,
                                               String providerUrl,
                                               String oidcIssuer) {
            ...
            synchronized (this) {
                ...
                if (localDecoder == null) {
                    ...
                    OAuth2TokenValidator<Jwt> audienceValidator = jwt -> {
                        ...
                        return OAuth2TokenValidatorResult.failure(
                                new OAuth2Error("invalid_audience", "The token does not contain the expected audience.", null));
                    };
                    NimbusJwtDecoder nimbusDecoder = JwtDecoders.fromIssuerLocation(oidcIssuer);
                    nimbusDecoder.setJwtValidator(JwtValidators.createDefaultWithValidators(List.of(issuerValidator, audienceValidator, forbiddenHeadersValidator)));
     ...
     */
    protected static boolean pinnedClaimMatches(String claim, Map<String, String> registered, Map<String, String> token) {
        String registeredValue = registered.get(claim);
        return registeredValue == null || registeredValue.equals(token.get(claim));
    }
