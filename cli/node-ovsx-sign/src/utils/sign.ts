...
const signFile = async (file: Buffer, privateKey: string): Promise<Buffer> => {
    const sign = crypto.createSign('sha256');
    sign.update(file);
    return sign.sign(privateKey);
}

const verifySignature = async (file: Buffer, publicKey: string, signature: Buffer): Promise<boolean> => {
