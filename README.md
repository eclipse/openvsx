...
The tests in [`tests/smoke-tests.spec.ts`](`tests/smoke-tests.spec.ts`) are designed to be executed against a local instance, a staging instance or the production instance at open-vsx.org.
For now, it just contains one configuration in [`playwright.config.ts`](playwright.config.ts), which is configured to run against open-vsx.org directly.
...
The smoke tests run against thr webui and can be executed in various modes, each suitable for different stages of development and debugging:
