...
function serviceStub(
    options: { status?: TrustedPublisherStatus; publishersByUrl?: Record<string, TrustedPublisher[]> } = {}
) {
    ...
    const getTrustedPublishers = vi
        .fn()
        .mockImplementation((_controller: AbortController, url: string) => Promise.resolve(publishersByUrl[url] ?? []));
    const service = {
        getNamespaces,
...
describe('UserSettingsTrustedPublishers', () => {
    it('aggregates the publishers of all manageable namespaces, sorted by namespace and extension', async () => {
        const { service } = serviceStub({
            publishersByUrl: {
                [URL_BETA]: [trustedPublisher({ id: 2, namespace: 'beta', extension: 'one' })],
                [URL_ALPHA]: [trustedPublisher({ id: 1, namespace: 'alpha', extension: 'one' })]
            }
        });
