...
function buildSearchUrl(q: string, cat: ExtensionCategory | '', sb: SortBy, so: SortOrder): string {
...
export function useSearchFilter() {
    ...
    handlerRef.current = (q: string) => {
        ...
        history.replaceState(null, '', buildSearchUrl(q, category, sortBy, sortOrder));
    };
    ...
    // wipe a URL-provided query (e.g. /search?q=vue) before navQuery has caught up.
    ...
    useEffect(() => {
        ...
        setDebounceTime(500);
        history.replaceState(null, '', buildSearchUrl(navQuery, category, sortBy, sortOrder));
    }, [navQuery]);
    ...
    const updateURL = (q: string, cat: ExtensionCategory | '', sb?: SortBy, so?: SortOrder) => {
        history.replaceState(null, '', buildSearchUrl(q, cat, sb ?? sortBy, so ?? sortOrder));
    };
