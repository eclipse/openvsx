...
- Add a `/publish` page and make the whole app a drop target for `.vsix` packages: dropping any number of them anywhere queues each one, moves to the publish page and uploads straight away, with no confirmation dialog. The page shows the queue as a line of extension cards — a skeleton while a package uploads, the real card once the registry accepts it — and keeps polling for anything left under review or still missing its icon
